...
var typeEq = (a, b) => {
  ...
  if(a.tag === TApp)
    return b.tag === TApp && typeEq(a.left, b.left) && typeEq(a.right, b.right);
...
var typeSubst = (t1, v, t2) => {
  ...
  if(t2.tag === TApp)
    return tapp(typeSubst(t1, v, t2.left), typeSubst(t1, v, t2.right));
...
var typewf = (c, t) => {
  ...
  if(t.tag === TApp) return typewf(c, t.left) && typewf(c, t.right);
...
var apply = (gamma, t) => {
  ...
  if(t.tag === TApp) return tapp(apply(gamma, t.left), apply(gamma, t.right));
...
var subtype = (state, gamma, t1, t2) => {
  ...
  throw new Error('subtype failed: ' + typeStr(t1) + ' and ' + typeStr(t2));
};
...
var instantiateL = (state, gamma, alpha, a) => {
  ...
  if(a.tag === TFun) {
    ...
    var r3 = instantiateR(
      r2.state,
      insertAt(gamma, cexists(alpha), context([
        cexists(alpha2),
        cexists(alpha1),
  ...
  if(a.tag === TApp) {
    ...
    var r3 = instantiateR(
      ...
      insertAt(gamma, cexists(alpha), context([
        cexists(alpha2),
        cexists(alpha1),
        cexistssolved(alpha, tapp(texists(alpha1), texists(alpha2))),
      ])),
      a.left,
      alpha1
    );
