...
const TImpl = (left, right) => ({ tag: 'TImpl', left, right });
...
const prune = t => {
  ...
  if (t.tag === 'TImpl') return TImpl(prune(t.left), prune(t.right));
...
const substTVar = (x, s, t) => {
  ...
  if (t.tag === 'TImpl')
    return TImpl(substTVar(x, s, t.left), substTVar(x, s, t.right));
  if (t.tag === 'TApp')
...
const subtypeTMeta = (tvs, m, t, left) => {
  ...
  if (t.tag === 'TMeta') {
    ...
    if (!subset(m.tvs, t.tvs))
      return terr(`subset failed: ${showTypeP(m)} := ${showTypeP(t)}`);
    m.type = t;
    return;
  }
  if (t.tag === 'TSkol') {
    if (!contains(m.tvs, t.id))
      return terr(`tvar out of scope: ${showTypeP(m)} := ${showTypeP(t)}`);
    m.type = t;
    return;
  }
  if (t.tag === 'TVar') {
  ...
  return terr(`subtype unexpected type: ${showTypeP(m)} := ${showTypeP(t)}`);
};
...
const subtype = (tvs, a, b) => {
  ...
  if (a.tag === 'TImpl') {
    constraints.push(a.left);
    subtype(tvs, a.right, b);
    return;
  }
  if (a.tag === 'TMeta') {
  ...
  return terr(`failed ${showTypeP(a)} <: ${showTypeP(b)}`);
};
...
const check = (env, tvs, t, ty) => {
  ...
  if (ty.tag === 'TImpl') {
    const v = freshVar();
    handledconstraints.push(ty.left);
    check(Cons([v.name, ty.left], env), tvs, t, ty.right);
    return;
  }
...
const synthapp = (env, tvs, ty, t) => {
  ...
  if (ty.tag === 'TImpl') {
    constraints.push(ty.left);
    return synthapp(env, tvs, ty.right, t);
  }
  if (ty.tag === 'TFun') {
  ...
  return terr(`cannot synthapp ${showTypeP(ty)} @ ${showTerm(t)}`);
};
...
const infer = (env, t) => {
  ...
  for (let i = 0, l = handledconstraints.length; i < l; i++) {
    const j = constraints.indexOf(handledconstraints[i]);
    if (j >= 0) constraints.splice(j, 1);
  }
  console.log(`constraints: [${constraints.map(prune).map(showType).join(', ')}]`);
  console.log(`handled constraints: [${handledconstraints.map(prune).map(showType).join(', ')}]`);
  return prune(ty);
};
...
const env = fromArray([
  ['id', tid],
  ['Z', tv('Int')],
  ['show', TForall('t', TImpl(TApp(tv('Show'), tv('t')), TFun(tv('t'),  tv('Str'))))],
]);

const term = Ann(Abs('x', App(v('show'), v('x'))), TForall('t', TImpl(TApp(tv('Show'), tv('t')), TFun(tv('t'),  tv('Str')))));
