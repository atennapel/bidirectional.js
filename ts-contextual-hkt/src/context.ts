...
import { Type, showType, isTVar, isTEx, isTFun, isTForall, isTApp, tforall, tfun, tapp, } from './types';
...
export const contextApply = (c: Context, t: Type): Type => {
  ...
  if(isTApp(t)) return tapp(contextApply(c, t.left), contextApply(c, t.right));
