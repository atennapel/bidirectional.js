...
export type Type = TVar | TEx | TApp | TFun | TForall;
...
export const tapps = (ts: Type[]): Type => ts.reduce(tapp);
...
export const flattenTApp = (t: TApp): Type[] => {
  const r: Type[] = [];
  let c: Type = t;
  while(isTApp(c)) { r.push(c.right); c = c.left }
  r.push(c);
  return r.reverse();
};
...
export const showType = (t: Type): string => {
  ...
  if(isTApp(t)) return flattenTApp(t).map(t =>
    isTFun(t) || isTForall(t) || isTApp(t)? `(${showType(t)})`: showType(t)).join(' ');
  if(isTFun(t)) return flattenTFun(t).map(t =>
    isTFun(t) || isTForall(t)? `(${showType(t)})`: showType(t)).join(' -> ');
  if(isTForall(t)) {
...
export const allTVar = (t: Type): Name[] => {
  if(isTVar(t)) return [t.name];
  if(isTEx(t)) return [];
  if(isTApp(t)) return allTVar(t.left).concat(allTVar(t.right));
  if(isTFun(t)) return allTVar(t.left).concat(allTVar(t.right));
  if(isTForall(t)) return [t.name].concat(allTVar(t.type));
  return impossible('allTVar');
};
export const freeTEx = (t: Type): Name[] => {
  if(isTVar(t)) return [];
  if(isTEx(t)) return [t.name];
  if(isTApp(t)) return freeTEx(t.left).concat(freeTEx(t.right));
...
export const containsTEx = (t: Type, n: Name): boolean => {
  ...
  if(isTApp(t)) return containsTEx(t.left, n) || containsTEx(t.right, n);
...
export const substTEx = (x: Name, s: Type, t: Type): Type => {
  ...
  if(isTApp(t)) return tapp(substTEx(x, s, t.left), substTEx(x, s, t.right));
...
export const substTVar = (x: Name, s: Type, t: Type): Type => {
  ...
  if(isTEx(t)) return t;
  if(isTApp(t)) return tapp(substTVar(x, s, t.left), substTVar(x, s, t.right));
...
export const isMono = (t: Type): boolean => {
  ...
  if(isTApp(t)) return isMono(t.left) && isMono(t.right);
